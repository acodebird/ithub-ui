  ...
  <div id="components-form-demo-advanced-search">
      ...
      <a-row :gutter="24">
        <a-col key="1" :span="8" >
          <a-form-item label="角色名字">
            <a-input allowClear v-decorator="['name']" />
          </a-form-item>
        </a-col>
        <a-col key="2" :span="8" >
   ...
   <div style="margin-bottom: 16px">
     <a-popconfirm title="确认删除所选数据?" okText="是" cancelText="否" @confirm="handleDeleteAll" >
      <a-button type="danger" :loading="loading" :disabled="hasPerm('role:batchDelete')">
        批量删除
      </a-button>
     </a-popconfirm>
     <a-divider type="vertical" />
     <a-button type="primary" @click="handleAdd" :disabled="hasPerm('role:add')">
        添加角色
    </a-button>
   </div>
    <s-table
      ...
      showPagination="auto"
    >
        <span slot="action" slot-scope="text, record" v-show="!hasPerm('role:edit')">
          <template>
            <a @click="handleDistribute(record)" title="分配权限"><a-icon type="exclamation-circle"/></a>
            <a-divider type="vertical"/>
            <a @click="handleEdit(record)" title="编辑角色"><a-icon type="edit"/></a>
            <a-divider type="vertical" v-show="record.name !='ROOT'"/>
            <a-popconfirm title="确认删除消息?" v-show="record.name !='ROOT'"  okText="是" cancelText="否" @confirm="handleDelete(record.id)" >
    ...
    <!-- <user-add ref="addModal" @ok="handleOk" /> 
     -->
     <update-resource ref="updateModal" @ok="handleOk"/>
     <role-add ref="addModal" @ok="handleOk" />
     <role-edit ref="editModal" @ok="handleOk" />
  </a-card>
  </a-locale-provider>
</template>
...
import { updateRole,deleteRole,deleteRoleBatch,getRoleList } from '@/api/role'
import UpdateResource from './form/UpdateResource'
import RoleAdd from './form/RoleAdd'
import RoleEdit from './form/RoleEdit'
 ...
 data () {
    return {
      ...
      loadData: parameter => {
        ...
        return getRoleList({ ...parameter, ...this.queryParam }).then(res => {
            if (res.success === true) {
              //加载用户权限用于屏蔽按钮
              this.loadPermission()
              ...
              this.$notification.error({message: `加载角色权限不足`})
              return parsePage()
   ...
   //多选操作监听函数
    ...
    handleDeleteAll() {
      if(this.selectedRowKeys=="") {
        ...
        deleteRoleBatch(this.selectedRowKeys).then(res => {
          console.log(`批量删除角色${this.selectedRowKeys}`)
          if(res.success === true) {
